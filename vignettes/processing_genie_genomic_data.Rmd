...
For the binary example we attempt to correlate the genomic features of the cohort with weather patients had also a brain metastasis. Let's first create the outcome:

```{r}
temp_outcome <- full_join(
  nsclc_stg_iv_adeno$cohort_ca_dx %>% 
    select(record_id,dmets_dx_brain),
  nsclc_stg_iv_adeno$cohort_cpt %>% 
    select(record_id, cpt_genie_sample_id),
  by = "record_id"
) %>% 
  select(cpt_genie_sample_id,dmets_dx_brain)
outcome_bin <- temp_outcome$dmets_dx_brain[match(rownames(bin_df_ex),temp_outcome$cpt_genie_sample_id)]
```
