...
```{r echo = FALSE, warning = FALSE, message = FALSE, eval=genieBPC:::.check_synapse_login()}
tibble::tribble(
...
```{r, eval = FALSE}
nsclc_data = pull_data_synapse("NSCLC", version = "v2.1-consortium")
```
...
```{r, echo = FALSE, message = FALSE, eval=genieBPC:::.check_synapse_login()}
ls(nsclc_data$NSCLC_v2.1) %>% 
  as.data.frame() %>% 
...
```{r, eval = FALSE}
pull_data_synapse(c("NSCLC", "CRC"), version = c("v2.1-consortium","v1.1-consortium"))
```
...
* As of May 2022, the non-small cell lung cancer (NSCLC), colorectal cancer (CRC), and breast cancer (BrCa) cohorts are available to access. Future cohorts will include pancreas, prostate and bladder cancers. The breast cancer cohort will be available to access late 2022.
