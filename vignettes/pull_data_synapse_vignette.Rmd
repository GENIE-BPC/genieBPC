...
```{r message = FALSE, warning = FALSE, eval=genieBPC:::check_synapse_login()}
...
```{r, eval = FALSE}
pull_data_synapse(c("NSCLC", "CRC"), version = c("2.1-consortium","1.1-consortium"))
```
...
pull_data_synapse(c("NSCLC", "CRC"),
                  version = c("1.1-consortium", "1.1-consortium"))
```
...
* As of April 2022, the non-small cell lung cancer (NSCLC), colorectal cancer (CRC), and breast cancer cohorts are available to access. Future cohorts will include pancreas, prostate and bladder cancers. The breast cancer cohort will be available to access late 2021.
