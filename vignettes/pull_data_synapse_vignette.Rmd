...
***Generate Synapse Personal Access Token (PAT)***

- On the Synapse dashboard, navigate to **Account Settings.**

- Towards the bottom of the page, select **Manage Personal Access Tokens.**

- Create a personal access token by clicking **Create New Token.**

***Authenticate yourself***

Whether you are using public or consortium data, you will need to authenticate yourself at the beginning of each R session in which you use {genieBPC} to pull data.  To set your Synapse credentials during each session, call `set_synapse_credentials()` using your PAT: <br>

<center>

`set_synapse_credentials(pat = "your_pat")` <br>

</center>
  
Alternatively, you may pass your Synapse username and password through `set_synapse_credentials()`: <br>

<center>

`set_synapse_credentials(username = "your_username", password = "your_password")` <br>

</center>

- **Note: connecting to Synapse via username and password will soon be deprecated. Instead, connecting via PAT is preferred.**
...
`SYNAPSE_PAT = <your-pat>` <br>
  
`SYNAPSE_USERNAME = <your-username>` <br>
  
`SYNAPSE_PASSWORD = <your-password>`

</center>

Alternatively, you can pass your PAT or username/password to each individual data pull function if preferred, although it is recommended that you manage your passwords outside of your scripts for security purposes.
...
ls(nsclc_2_0$NSCLC_v2.0) %>%
  as.data.frame() %>%
  mutate(clinical_vs_genomic = case_when(
    . %in% c("cna", "fusions",
