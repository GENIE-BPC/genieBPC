...
The `GenieBPC` includes functionalities to link the clinical records of cohorts of interest to patients corresponding genomic samples. In this tutorial we follow the data that was downloaded in the `Tutorial: pull_data_synapse()` Vignette, and processed in the `Tutorial: create_cohort` Vignette, as shown below:
...
- cohort_object: the object outputted by the `pull_data_synapse()` function.
- df_record_ids: the `cohort_ca_drugs` object returned by the `create_cohort()` function.
...
Note that this object is returned from the `create_cohort()` object under the name `cohort_cpt`:

```{r}
gt(nsclc_stg_iv_adeno$cohort_cpt[1:10,])
```
...
- sample type: primary or metastatic tumor, in our case "Metastasis".
...
```{r}
optimal_samples <- select_unique_sample(samples_object = samples_data, histology = "LUAD",sample_type = "Metastasis",min_max_time = "min")
```
